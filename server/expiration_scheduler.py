...
def get_global_scheduler_settings() -> dict:
    ...
    try:
        ...
        if result and result.get("extra_info"):
            ...
            check_time = extra_info.get("expiration_check_time", f"{DEFAULT_CHECK_HOUR:02d}:{DEFAULT_CHECK_MINUTE:02d}")
...
def get_store_scheduler_settings(store_id: int) -> dict:
    ...
    try:
        ...
        if result and result.get("extra_info"):
            extra_info = result["extra_info"]
            alert_days = extra_info.get("expiration_alert_days", DEFAULT_EXPIRATION_ALERT_DAYS)
            check_time = extra_info.get("expiration_check_time", f"{DEFAULT_CHECK_HOUR:02d}:{DEFAULT_CHECK_MINUTE:02d}")
            parts = check_time.split(":")
            check_hour = int(parts[0]) if len(parts) >= 1 else DEFAULT_CHECK_HOUR
            check_minute = int(parts[1]) if len(parts) >= 2 else DEFAULT_CHECK_MINUTE
...
def get_store_expiration_threshold(store_id: int) -> int:
    ...
    try:
        ...
        if result and result.get("extra_info"):
            extra_info = result["extra_info"]
            return extra_info.get("expiration_alert_days", DEFAULT_EXPIRATION_ALERT_DAYS)
...
def check_expiring_products_for_store(store_id: int, threshold_days: int):
    ...
    """
    logging.info(f"Starting expiration check for store {store_id} with threshold {threshold_days} days...")
    ...
    try:
        ...
        for product in expiring_products:
            ...
            logging.info(f"Updated expiration notification for product {product_id} in store {store_id}")
        ...
        for product_id in products_to_remove:
            ...
            logging.info(f"Removed expiration notification for product {product_id} in store {store_id}")
    ...
    while True:
        try:
            ...
            if last_logged_schedule != schedule_key:
                ...
                logging.info(f"Store {store_id}: Next expiration check at {next_run.strftime('%Y-%m-%d %H:%M')} (in {sleep_seconds/3600:.1f} hours)")
            ...
            if current_time == scheduled_time and last_scheduled_run != current_scheduled_key:
            ...
            logging.error(f"Error in expiration scheduler loop for store {store_id}: {e}")
...
async def expiration_scheduler_loop():
    ...
    while True:
        try:
            ...
            sleep_seconds = (next_run - now).total_seconds()
            logging.info(f"Next expiration check scheduled at {next_run.strftime('%Y-%m-%d %H:%M')} (in {sleep_seconds/3600:.1f} hours)")
...
def start_expiration_scheduler():
    ...
    try:
        ...
        for store in stores:
            ...
            logging.info(f"Expiration scheduler background task created for store {store_id}")
