...
const Bills = () => {
  ...
  const handleExportToPdf = async (mode: "summary" | "full") => {
    try {
      // De-duplicate the rendered list: expanded mode (and bill-id search)
      // interleaves a twin "expanded" row after every real bill.
      const reportBills =
      ...
      const html = buildBillsReportHtml({
        ...
        endDate: hasActiveBillIdSearch ? "-" : endDate.format("YYYY/MM/DD HH:mm"),
        typesLabel,
        partyLabel,
        productLabel,
      ...
      const result = await exportPdfDocument({
        fileName: `bills-${mode}-${startDate.format("YYYY-MM-DD")}.pdf`,
        html,
        ...
        <Grid2 container size={12} spacing={2}>
          ...
          <Grid2 size={{ xs: 6, sm: 3 }}>
            <StatCard
              ...
              info="مجموع حقل الإجمالي المسجَّل للفواتير الظاهرة بالفلتر الحالي (البيع موجب، والشراء والمرتجع سالب، والآجل والتقسيط مسجَّلة بصفر). يتغيّر حسب أنواع الفواتير المختارة."
            />
          </Grid2>
          <Grid2 size={{ xs: 6, sm: 3 }}>
            <StatCard
              ...
              info="مثل إجمالي الفواتير، لكن يُحتسب لفواتير الآجل والتقسيط قيمتها الفعلية من أسعار المنتجات بدل الصفر. يعكس الفلتر الحالي وقد يشمل أنواعاً غير البيع، فقد يختلف عن إجمالي المبيعات في صفحة التحليلات."
            />
          </Grid2>
          <Grid2 size={{ xs: 6, sm: 3 }}>
            <StatCard
