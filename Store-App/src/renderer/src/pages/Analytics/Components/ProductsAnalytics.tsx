...
const getAnalytics = async (startDate: string, endDate: string, selectedProducts: number[]) => {
...
const getProds = async () => {
  const { data } = await axios.get<DBProducts>(import.meta.env.VITE_SERVER_URL + '/products')
  return data
}
...
const ProductsAnalytics = () => {
  ...
  const { data, isFetching } = useQuery({
    ...
    queryFn: () => getAnalytics(startDate.toISOString(), endDate.toISOString(), selectedProducts),
    initialData: {},
          ...
          myTool: {
            ...
            onclick: () => {
              ...
              const exportData = [
                ...
                ...Array.from(groupedByDate.entries()).flatMap(([date, products]) =>
                  Array.from(products.entries()).map(([name, value]) => [date, name, value])
                )
              ]
      ...
      <Card elevation={3} sx={{ px: 3, py: 2, position: 'relative' }}>
        <Grid container spacing={2}>
          <Grid item xs={12}>
            ...
            <Typography variant="body1">قم بتحديد الفترة و المنتجات لعرض الاحصائيات</Typography>
          ...
          <Grid item xs={12}>
            <Autocomplete
              ...
              value={products.products.filter((prod) => selectedProducts.includes(prod.id!))}
              onChange={(_, newValue) => setSelectedProducts(newValue.map((prod) => prod.id!))}
              renderInput={(params) => <TextField {...params} label="المنتجات" />}
            />
          </Grid>
          <Grid item xs={12}>
            <EChartsReact option={options} style={{ height: 500 }} theme={mode} notMerge={true} />
          </Grid>
        </Grid>
      </Card>
    </Grid>
