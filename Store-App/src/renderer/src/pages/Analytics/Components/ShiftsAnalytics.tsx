...
const getAnalytics = async (startDate: string, endDate: string, types: string[]) => {
...
const ShiftsAnalytics = () => {
  ...
  const [requiredTypes, setRequiredTypes] = useState<string[]>(['sell', 'return'])
  ...
  const { data, isFetching } = useQuery({
    ...
    queryFn: () => getAnalytics(startDate.toISOString(), endDate.toISOString(), requiredTypes),
    initialData: []
  })
      ...
      <Card elevation={3} sx={{ px: 3, py: 2, position: 'relative' }}>
        <Grid container spacing={2}>
          <Grid item xs={12}>
            ...
            <Typography variant="body1">قم بتحديد الفترة لعرض الاحصائيات</Typography>
            <Typography variant="body1">الشيفت الحالى غير مضمون فى التقرير حتى اغلاقة</Typography>
          </Grid>
